...
bool checkWhiteConnected(vector<vector<Cell>> board) {
    ...
    if(i1 == -1) { //if there are no non-black cell, return true
        return true;
    }
...
vector<vector<Cell>> generate_board(int w, bool debug_mode) {
    ...
    //Fill in the board with unmarked cell with latin square numbers
    for(int i = 0; i < w; i++) {
        vector<Cell> m(w);
    ...
    for(int i = 0; i < (w * w / 2); i++) {
        ...
        //not to black this cell is any of its neighbor cell are black
    ...
    //the question and the answer will be outputted into the log file if debug mode enabled
    if(debug_mode) {
        ofstream of;
...
//check if there are any duplicated number and any rows
...
//check if there are any duplicated number and any column
