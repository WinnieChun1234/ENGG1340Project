...
bool check_white(vector<vector<Cell>> board, bool out) {
    if(!check_white_connected(board)) {
        if(out) {
...
vector<vector<Cell>> generate_board(int w, bool debug_mode) {
    ...
    for(int i = 0; i < (w * w / 2); i++) {
        ...
        if(check_white_connected(board)) { //if all white cells in the board are connected, change the number of the cell
...
bool check_rows(vector<vector<Cell>> board, bool out) {
...
//board: the board to be checked
//out: enable/disable logging onto screen
bool check_columns(vector<vector<Cell>> board, bool out) {
    //create a vector of size "board[0].size()" filled with "false"
...
//board: the board to be checked
//out: enable/disable logging onto screen
bool check_adj_black_cell(vector<vector<Cell>> board, bool out) {
